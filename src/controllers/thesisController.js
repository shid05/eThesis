...
// Looks up the thesis, asks Cloudinary for the resource details
// (cloudinary.api.resource), then streams the PDF back to the client
// as an attachment so the browser actually triggers a download.
const download_get = async (req, res) => {
  try {
    ...
    const thesis = await Thesis.findById(req.params.id).populate('author', 'name');
    if (!thesis) return res.status(404).send('Thesis not found');
    if (!thesis.fileUrl) return res.status(404).send('No file attached to this thesis');

    // Visibility rules — same as detailApi
    const isOwner = req.session?.user?.id === thesis.author._id.toString();
    const isTeacher = req.session?.user?.role === 'Teacher';
    const isAdmin = req.session?.user?.role === 'Admin';
    const isApproved = thesis.status === 'Approved';

    if (!isApproved && !isOwner && !isTeacher && !isAdmin) {
      return res.status(403).send('You do not have access to download this thesis');
    }

    // Pull the public_id out of the stored Cloudinary URL
    const publicId = extractPublicIdFromUrl(thesis.fileUrl);
    if (!publicId) {
      console.error('Could not parse public_id from URL:', thesis.fileUrl);
      return res.status(500).send('Invalid file URL');
    }

    // === The piece the user asked for ===
    // cloudinary.v2.api.resource(publicId).then(result => ...)
    // PDFs are uploaded as resource_type "raw", so we ask Cloudinary
    // for the raw resource details (asset_id, bytes, secure_url, format, ...).
    ...
    // Build a friendly filename like "My_Thesis_Title.pdf"
    const safeTitle = (thesis.title || 'thesis')
      .replace(/[^a-zA-Z0-9\-_\s]/g, '')
      .trim()
      .replace(/\s+/g, '_') || 'thesis';
    const ext = resource.format || 'pdf';
    const filename = `${safeTitle}.${ext}`;

    // Stream the file from Cloudinary -> client, forcing a download
    ...
    client
      .get(fileUrl, (cloudRes) => {
        ...
        if (cloudRes.headers['content-length']) {
          ...
          res.setHeader('Content-Length', resource.bytes);
        }
    ...
    if (err.http_code === 404 || err.error?.http_code === 404) {
      return res.status(404).send('File not found on Cloudinary');
    }
